...
export type FlightListAction =
  ...
  | { type: 'SELECT_FLIGHT'; payload: { flight: Flight; direction: 'Outbound' | 'Return' } }
...
// Reducer
export const flightListReducer = (state: FlightListState, action: FlightListAction): FlightListState => {
  switch (action.type) {
    case 'SET_FLIGHTS':
    ...
    case 'SELECT_FLIGHT': {
      ...
      if (direction === 'Outbound') {
        if (state.trip.return && isAfter(flightDate, parseISO(state.trip.return.date))) {
          newTrip.return = null;
        }
        newTrip.outbound = flight;
      } else {
        if (state.trip.outbound && isBefore(flightDate, parseISO(state.trip.outbound.date))) {
    ...
    case 'APPEND_FLIGHTS': {
      ...
      const uniqueFlightsMap = new Map(combinedFlights.map(flight => [flight.uuid, flight]));
      return {
        ...state,
